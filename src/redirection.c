...
int execute_with_redirection(command_t* cmd) {
    ...
    if (pid == 0) {
        ...
        return cmd->background ? 0 : WEXITSTATUS(status);
    } else {
        perror("fork");
        return -1;
    }
...
int execute_pipeline(pipeline_t* pipeline) {
    ...
    // Check if any command should run in background
    ...
    if (pipeline->num_commands == 1) {
        ...
        // Regular command - handle background execution
        if (cmd->background) {
            pid_t pid = fork();
            if (pid == 0) {
                // Child process
                ...
                // Parent process - don't wait for background job
                printf("[Background] %d\n", pid);
                return 0;
            } else {
                perror("fork");
    ...
    // Multiple commands with pipes - run in foreground only
    if (run_in_background) {
        printf("Note: Pipelined commands running in foreground\n");
    }
